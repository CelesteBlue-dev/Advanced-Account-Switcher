...
void accountMenu() {
	...
	switch (get_key()) {
		...
		case SCE_CTRL_CROSS:
			...
			psvDebugScreenClear(0);
			pos = 3;
			if (!ret) printf("Written to registry successfully!\n");
...
void saveAccount() {
	sceIoMkdir("ux0:/AdvancedAccountSwitcher", 6);
	sceIoMkdir("ux0:/AdvancedAccountSwitcher/accounts", 6);
	char path[256];
	char username[256];
...
void main() {
	...
	if (!strlen(username)) printf("Current Account: None!\n");
	...
	switch(get_key()) {
		...
		case SCE_CTRL_CROSS:
			if (!pos)
				...
				sceAppMgrLaunchAppByUri(0x40000, "psnreg:");
			goto again;
		default:
			goto again;
	}
